...
/*        if (safe.binds[name]) {
  ...
  carena.addFeature("carena.Style", function(obj, options, storage) {
    ...
    function defineEventedProperty(store, target, name) {
      var setter = function(value) {
            oldValue = store[name] || null;
            store[name] = value;
          ...
          getter = function() { return store[name] || null; };
    ...
    for (i=0; i<l; i++) {
      storage.style[properties[i]] = 0;
      defineEventedProperty(storage, obj.style, properties[i]);
    }
  ...
  carena.addFeature("carena.Box", function(obj, options, storage) {
    ...
    carena.applyProperties(obj.style, {
      get innerHeight() {
        return storage.innerHeight;
      },
      get innerWidth() {
        return storage.innerWidth;
      }
    });

    // Events
    function recalcInnerWidth() {
      var innerWidth    = storage.width -
    ...
    function recalcInnerHeight() {
      var innerHeight    = storage.height -
