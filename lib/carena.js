(function(exports) {
  ...
  carena.addFeature("carena.Renderable", function(obj, options, storage) {
    ...
    obj.event.bind("node.*", function(name, data) {
      if (data.node === obj) {
        var actual = name.split(".").pop();
        if (last[actual]) {
          last[actual] = data.value;
        }
      }
    });
    ...
    return carena.applyProperties(obj, {
      clear : function(renderer) {
        ...
        renderer.context.fillRect(last.x, last.y, last.width, last.height);
      },
      render : function(renderer) {
        renderer.context.fillStyle = obj.color || "white";
        renderer.context.fillRect(obj.x, obj.y, obj.width, obj.height);
      }
    });
  });
  ...
  carena.addFeature("carena.Node", function(obj, options, storage) {
    ...
    options = options || {};
    storage = storage || {};
    storage.x = options.x || 0;
    storage.y = options.y || 0;
    storage.z = options.z || 0;
    storage.width = options.width || 0;
    storage.height = options.height || 0;
    storage.style = storage.style || options.style || { backgroundColor : options.color || "white" };
    ...
    return carena.applyProperties(obj, {
      ...
      get color() { return storage.style.backgroundColor; },
      set color(value) {
        if (!storage.style) {
          storage.style = {};
        }
  ...
  carena.addFeature("carena.Renderer", function(obj, options, storage) {
    ...
    carena.applyProperties(obj, {
      ...
      renderTree : function(root) {
        ...
        return root.walk(function(node, callback) {
  ...
  carena.addFeature("carena.Camera", function(obj, options, storage) {
    var safe = {
      ...
      clickTimeout : options.clickTimeout || 500
    };
    ...
    function mouseDown(ev) {
      ...
      if (safe.mouseDownTarget) {
        ...
        if (safe.mouseDownTarget.lastClick &&
            currentTime - safe.mouseDownTarget.lastClick < safe.clickTimeout)
        {
        ...
        if (safe.mouseDownTarget.clickCount > 1) {
          eventName += "." + safe.mouseDownTarget.clickCount
        }
    ...
    function mouseUp(ev) {
      ...
      if (safe.mouseDownTarget) {
        ...
        safe.mouseDownTarget.lastClick = currentTime;

        if (safe.mouseDownTarget.clickCount > 1) {
          eventName += "." + safe.mouseDownTarget.clickCount
        }

        safe.mouseDownTarget.event.trigger(eventName, {
          target: safe.mouseDownTarget,
          collisions : safe.targets,
          mouse: safe.mouse
        });
