  ...
  carena.feature = {
    Node : function(obj, options) {
      ...
      return carena.applyProperties(obj, {
        ...
        set parent(value) {
          ...
          if (obj.event) {
            /*fobj.event.trigger("node.parent", {
							node    : obj,
							current : safe.parent,
						  next: value
						});*/
      ...
      carena.applyProperties(obj, {
        ...
        renderTree : function(node) {
          return node.walk(function(node, callback) {
            ...
            /*
            TODO: optimize me.
            currently, dirty propagates up the tree, but in reality it needs to go down
						the tree as well for this to be "optimized".  Adding to this problem, we
					  have a trigger on every dirty so things get recursive.
    ...
    Camera : function(obj, options) {
      ...
      function mouseMove(ev) {
        ...
        targets = safe.target.nodesByPoint(safe.mouse.x, safe.mouse.y)
				l = targets.length;
    ...
    Draggable : function(obj, options) {
      ...
      // Mouse movement is tricky.  On one hand, you don't want to continue sending
      // mouse.move to a node after you have mouse.out'd. On the other hand, dragging
      // items around can be slow, and may experience a few mouse.out events.
    ...
    Droppable : function(obj, options) {

       // - detect mouse.in, out, up
       //  - bind to drag.move
       //   - on drag.move, fire drop.move
       // - detect mouse.out
       //   - unbind drag
       //   - fire drop.cancel
       // - on mouse.up
       ...
       // Problems:
       // - different types of drop, that do not depend on the mouse being inside the node.
       return carena.applyProperties(obj, {
         get droppabl() { return true; }
       });
    },

    DropTarget : function(obj, features, options) {
    ...
    DropManager : function(obj, options) {
       ...
       obj.event.bind("drag.end", function(name, data) {
         ...
         // TODO: using obj.target is hackery, fix
         var dropTargets = obj.target.nodesByPoint(data.mouse.x, data.mouse.y);
         var i=0;l=dropTargets.length;
         for (i; i<l; i++) {
           if (dropTargets[i].myId !== obj.target.myId) {
             debugger;
             data.target.parent.remove(data.target);
             dropTargets[i].add(data.target);
             data.target.color="orange";
             break;
           }
         }
       });
