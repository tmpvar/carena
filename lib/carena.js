      ...
      obj.event.bind("node.parent", function(name, data) {
       if (data.node === obj) {
         if (data.current && data.current.event) {
           data.current.event.unbind("node.x", obj.positionChanged);
           data.current.event.unbind("node.y", obj.positionChanged);
         }

         if (data.next && data.next.event) {
           data.next.event.bind("node.x", obj.positionChanged);
           data.next.event.bind("node.y", obj.positionChanged);
         }
       }
      });
      ...
      return carena.applyProperties(obj, {
        positionChanged : function(name, data) {
          if (data.node === obj.parent) {
            var diff = (data.value - data.oldValue);
            switch (name) {
              case "node.x":
                obj.x += diff;
              break;
