(function(exports) {
  ...
  carena.addFeature("carena.Renderable", function(obj, options, storage) {
    ...
    carena.applyProperties(obj, {
      ...
      render : function(renderer) {
        var i=0, steps = obj.renderSteps, l = steps.length;
        if (l>0) {
          for (i; i<l; i++) {
    ...
    obj.event.bind("mouse.down", function(name, data) {
      ...
      for (i=l-1; i>=0; i--) {
        ...
        if (c && c.hasFeature("carena.Draggable")) {
          ...
          break;
          /*
          safe.node = c.dragstart(c, data.mouse);
  ...
  carena.addFeature("carena.Backbuffer", function(obj, options, storage) {
    ...
    safe.canvas.width=9000;
    safe.canvas.height=9000;


safe.canvas.style.border = "1px solid black";
    ...
    obj.event.bind("node.dirty", function(name, data) {
      //safe.canvas.width = 600;
      //safe.canvas.height = 600;
    });
    ...
    return carena.applyProperties(obj, {
      render : function(renderer) {
        ...
        obj.descend(function(node) {
          ...
          if (node === obj) {
            _render(safe.renderer);
          } else {
            node.render(safe.renderer);
          }
          ctx.restore();
        });
      ...
      renderBackbuffer : function(renderer, backbuffer) {
        var data = backbuffer.getImageData(0, 0, obj.width, obj.height);
